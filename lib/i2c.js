...
class BNO08X_I2C extends BNO08X {
    ...
    constructor(i2c_bus_number, options = {}) {
        ...
        // FIXME: async work hidden in a constructor with no way for callers to
        // await it or handle errors at construction time. This should be an
        // explicit async open() method that mirrors close(). For now,
        // _initPromise is stored // so enable_feature() can guarantee the reset
        // cycle completes before use.
        this._initPromise = new Promise((resolve, reject) => {
            setImmediate(() => this.initialize().then(resolve).catch(reject));
        });
    }
